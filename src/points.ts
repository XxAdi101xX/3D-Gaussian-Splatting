...
export class Points {
	...
	constructor(
		...
		const positions = new Float32Array(gaussianSplats.flatMap(v => [v.position[0], v.position[1], v.position[2]]));
		const colors = new Float32Array(gaussianSplats.flatMap(_ => [0, 0, 1])); // defaulted to blue
